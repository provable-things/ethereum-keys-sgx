...
pub fn run(address: &Address, message: String, signature: String) -> Result<bool> {
    Ok(&public_to_address(recover_public_key(message, signature)?)? == address) 
}

fn recover_public_key(message: String, signature: String) -> Result<PublicKey> {

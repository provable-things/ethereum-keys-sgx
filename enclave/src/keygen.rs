...
//         let tx = tx.clone();
//         pool.execute(move || {
//             let pref = prefix.from_hex().expect("Error: valid hex required for prefix!"); Orig. ver using rustc_hex::ToHex;
//             // let pref = hex!(prefix); // macro version!
